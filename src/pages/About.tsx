...
const teamMembers = [
  ...
  },
  {
    ...
    description: 'Aluno de Engenharia de Computação no CEFET Petrópolis e residente no Serratec/Senac. Atualmente procurando desenvolver minhas habilidades em desenvolvimento de softwares.',
  ...
  },
  {
    ...
    description: 'Flávio Lourenço, apaixonado por tecnologia desde a infância, atualmente migrando para uma carreira em desenvolvimento de software Full Stack. No Senac, estou aprimorando minhas habilidades técnicas e comportamentais, construindo seu futuro com determinação e entusiasmo.',
    ...
    <div className="container">
      ...
      {teamMembers.map((member, index) => (
        <div key={index} className="member-container">
          <img className="profile-image" src={member.imgSrc} alt={member.name} />
          <h2 className="name">{member.name}</h2>
          <p className="description">{member.description}</p>
          <div className="links-container">
            <LinkButton url={member.githubUrl} iconClass="fab fa-github" />
            <LinkButton url={member.linkedinUrl} iconClass="fab fa-linkedin" />
          </div>
          {index < teamMembers.length - 1 && <hr className="divisor" />}
        </div>
      ))}
